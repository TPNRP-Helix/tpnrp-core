...
DAO.inventory.save = function(inventory)
    ...
    local inventories = inventory.inventories
    local formattedInventories = {}
    for _, item in pairs(inventories) do
        local item = SHARED.items[item.name:lower()]
        if item then
            -- Only format the item if it is in the shared/items.lua
            formattedInventories[#formattedInventories + 1] = {
                name = item.name,
                amount = item.amount,
                info = item.info,
                slot = item.slot,
    ...
    local params = {
        inventory.type,
        citizen_id,
        JSON.stringify(formattedInventories),
    }
...
DAO.inventory.get = function(citizen_id, type)
    ...
    return JSON.parse(inventory.items)
end